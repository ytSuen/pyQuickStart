...
### 智能防休眠防锁屏（v1.3.0 增强）
- 💤 **独立防休眠控制**（可独立开启/关闭）
- 🎚️ **可配置防护强度**（轻度/中度/重度三档可选）
- 🖱️ **智能鼠标移动**（20-100像素可配置，规避自动锁屏）
- 🛡️ **四重防护机制**：
  - 鼠标智能移动（防锁屏，幅度可配置）
  - 重置空闲计时器（防休眠）
  - 模拟按键操作（防休眠）
  - 持续状态保持（防休眠）
- ⏱️ **智能刷新间隔**（15-60秒可配置，确保持续有效）
- 📊 **锁屏状态检测**（自动检测并记录锁屏事件）
- 🎯 **无感操作**（移动幅度可调，不影响使用）
- 🧪 **测试功能**（一键测试防锁屏效果）
...
| 防锁屏效果不佳 | 1. 尝试切换到"重度"防护强度<br>2. 点击"测试防锁屏"按钮检查状态<br>3. 查看日志了解详细信息<br>4. 参考[防休眠问题排查.md](./防休眠问题排查.md) |
| 锁屏仍然发生 | 1. 确认是锁屏还是休眠（参考文档）<br>2. 检查Windows锁屏设置<br>3. 使用"重度"防护强度<br>4. 查看锁屏统计信息 |
...
### v1.3.0 (2026-01-23)
- ✨ **新增可配置防护强度**（轻度/中度/重度三档可选）
- ✨ **智能鼠标移动**（20-100像素可配置，更有效防止锁屏）
